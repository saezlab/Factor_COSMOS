...
Firstly, we can calculate cell-line specific transcriptomic values by multiplying the MOFA factor weight (here: Factor 4) with the 'raw' transcriptomic values.

```{r Calculate cell-line specific transcriptomics}
# RNA cell-line-specific values (RNA_specific = MOFA_factor_weight * RNA_raw)
...
## Get MOFA factor weights from Factor 4  
...
RNA_weight_cell_line <- cbind(RNA_weight_cell_line, "final" = (RNA_weight_cell_line$value.x*RNA_weight_cell_line$value.y))
colnames(RNA_weight_cell_line) <- c("feature","sample","view","RNA_raw","RNA_MOFA","Final")
...
plot_factor(model, 
  ...
  geom_label(label=model@samples_metadata$sample, show.legend = F, position=position_dodge(width=0.9))
```
...
for(i in unique(RNA_weight_cell_line$sample)) {
  RNA_sample <- RNA_weight_cell_line[RNA_weight_cell_line$sample == i,6]
  names(RNA_sample) <- RNA_weight_cell_line[RNA_weight_cell_line$sample == i,1]
...
activity_scores <- rbind(lig_rec_scores_filtered_df,dorothea_scores_filtered_df[!(rownames(dorothea_scores_filtered_df) %in% c(rownames(moon_scores_filtered_df), rownames(lig_rec_scores_filtered_df)))], moon_scores_filtered_df[!(rownames(moon_scores_filtered_df) %in% c(rownames(dorothea_scores_filtered_df), rownames(lig_rec_scores_filtered_df))),])

activity_scores[,1] <- rownames(activity_scores)
...
setNodeShapeMapping(table.column = "NodeType", table.column.values = list("1","2","3"),shapes = list("Octagon","VEE","Triangle"), style.name = style.name, network = paste0(i, "_EGF_EGFR_RAS_ERK_pathway_start_end"))
