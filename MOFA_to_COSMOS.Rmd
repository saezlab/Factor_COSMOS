...
both look for correlation across samples AND across features
```{r Supp figure condition_prot_RNA_correlation}
...
```{r Supp figure single_prot_RNA_correlation}
...
Only run this chunk if you want to re-perform the mofa optimisation. The results are already available in the 
results/mofa folder. We are testing various amount of maximum number of factors that MOFA is allowed to explore.
...
in a terminal to instal the module to python3.
...
```{r Chekc cross correlation of omics compared to variance epxlained}
...
Next, we can characterize the factors with the available metadata of samples. Indeed, it can be interesting to see if some factors are associated with metadata classes, as they can help to guide our intepretation of the factors.
This part rely on an interpretation of the Z matrix of the mofa model, while ignoring the factor weight themselves. The factor weights will be analysed subsequently.
...
```{r Metabs across factors, fig.width=4, fig.height=2.2}
...
These activtiy estiamtions are saved in an input file for further downstream analysis as well.
...
```
Coherently, only the factors where there is variance explained for both RNA and proteins are correlated.
...
Thus, we first define the upstream input as receptors, and the downstream input as TFs and metabolites.
Since the scoring of the network is sensitive to the scale of the downstream measurments, we are multiplying 
the metabolite weights by 10 so that they are on a similar scale as the TF scores. Other scaling methods can be performed at the user'S discretion,
...
This part is only display for reference purposes, but is optional. If you wish to run the latest verison of the analysis with your data,
we advise to run instead the new MOON function, that can be found later in this tutorial.
...
This method is easier to set up and interpret than the classic cosmos run that 
relies on CPLEX and CARNIVAL.
...
```{r Prepare moon input for receptor to TFs and metabs (MOON)}
...
into a single compbined scored network. WE also add the conecitons between 
