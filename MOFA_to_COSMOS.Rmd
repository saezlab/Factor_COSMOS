...
moon_weights <- moon_weights[!(names(moon_weights) %in% c(rec_weights$Nodes,TF_weights$Nodes, lig_weights$Nodes))]
moon_weights <- data.frame(Nodes = names(moon_weights), feature_weights = moon_weights)

## Combine
feature_weights <- as.data.frame(rbind(TF_weights, rec_weights, lig_weights, moon_weights))
...
To identify whether a node is a ligand, receptor, TF or master regulator, we first load the consensus PKNs and only keep entries which represent an interaction in our network.
...
Then, we add the information to the network (ligand = 2, receptor = 3, transcription factor = 4, master regulator = 5, other = 1, no NodeType available = 0).
