...
mofa_ready_data_RNA <- mofa_ready_data[mofa_ready_data$view == "RNA",-3]
save(mofa_ready_data_RNA, file = "data/RNA/mofa_ready_data_RNA.RData")
```
...
save(Z_matrix, file = "results/mofa/z_matrix.RData")
save(all_metadata, file = "support/all_metadata.RData")
...
```
```{r check qunatiles of weights}

RNA_all_zscore <- as.data.frame(apply(RNA_all,2,function(x){scale(x)}))
row.names(RNA_all_zscore) <- row.names(RNA_all)
pnorm(RNA_all_zscore[c("MAPK1","PRKCA","ABL1","JAK1"),"Factor4"])
...
save(weights_RNA, file = "results/mofa/weights_RNA.RData")
