...
for(cell_line in names(cosmos_inputs))
{
  ...
  cell_line <- gsub("[^[:alnum:]]","",cell_line)
  ...
  {
    write(paste("results/decoupleRnival/",paste(cell_line, " failed",sep = ""), sep = ""), file = paste("results/decoupleRnival/",paste(cell_line, "_failure.txt",sep = ""), sep = ""))
  }
