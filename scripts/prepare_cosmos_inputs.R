...
cosmos_inputs <- sapply(names(TF_activities), function(cell_line, RNA_scaled, metabs_scaled, TF_activities_scaled){
  ...
  cosmos_input[["TF_scores"]] <- TF_activities_scaled[[cell_line]]
  return(cosmos_input)
}, RNA_scaled = RNA_scaled, metabs_scaled = metabs_scaled, TF_activities_scaled= TF_activities_scaled, USE.NAMES = T, simplify = F)
