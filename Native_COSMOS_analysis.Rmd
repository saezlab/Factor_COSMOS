...
for(i in c("NCI-H226","SF-539", "MOLT-4")) { 
...
for (cell_line in c("NCI-H226", "SF-539", "MOLT-4")) {
  ...
  ## TF
  TF_weights <- dorothea_scores_native
  ...
  ## mOOn
  moon_weights <- moon_scores_native
  ...
  # Load meta footprint analysis (moon) regulatory network 
  moon_regulons <- moon::build_moon_regulons()
  moon_regulons <- moon_regulons[moon_regulons$source %in% combined_ATT$Nodes,]
  
  combined_ATT$NodeType <- ifelse(combined_ATT$Nodes %in% ligrec_df$Node1, 2, ifelse(combined_ATT$Nodes %in% ligrec_df$Node2, 3, ifelse(combined_ATT$Nodes %in% dorothea_df$source, 4, ifelse(combined_ATT$Nodes %in% moon_regulons$source, 5, combined_ATT$NodeType)))) #if node is a ligand set NodeType to 2, if node is a receptor set NodeType to 3, if node is a TF set NodeType to 4, if node is a master regulator set NodeType to 5, else keep 0 or 1
...
for (cell_line in c("SF-539", "MOLT-4", "NCI-H226")) {
  ...
  setNodeShapeMapping(table.column = "NodeType", table.column.values = list("1","2","3","4","5"),shapes = list("Octagon","VEE","Triangle","Diamond","Ellipse"), style.name = style.name, network = paste0("Full COSMOS native network ", cell_line))
...
for (cell_line in c("SF-539", "MOLT-4", "NCI-H226")) { 
  ...
  setNodeShapeMapping(table.column = "NodeType", table.column.values = list("1","2","3","4","5"),shapes = list("Octagon","VEE","Triangle","Diamond","Ellipse"), style.name = style.name, network = paste0("Native COSMOS EGF_EGFR_RAS_ERK_pathway_start_end ", cell_line))
  setNodeColorDefault("#89D0F5", style.name)
